...
class BuildingsDataset(Dataset):
    ...
    def __init__(
        ...
        self.slide_resize = slide_resize
    ...
    def sample_patches(self, label_dis: list): 
        ...
        while idx < (self.num_patches*len(self.image_paths)):
            ...
            if str(filename) in self.slide_objects:
                ...
                patches[idx] = {"coords": (x,y), "slide_filename": filename}
                idx += 1
        
        return patches


    #read region function
    def get_patch(self, x: int = 0, y: int = 0, slide: object = None): 
        #resize 
        if self.slide_resize == True: # read region (doubled size)
            rgb = np.array(slide.read_region(location=(int(x * self.down_factor), int(y * self.down_factor)),
                                              level=self.level, size=(self.patch_size*self.down_factor, self.patch_size*self.down_factor)))[:, :, :3]
            rgb = rgb[::2, ::2]
    ...
    def get_y_patch(self, x: int = 0, y: int = 0, slide: object = None, polygons: list = None):
        ...
        #if contains tumor, overrite necrosis 
        ...
        if self.white_mask_application == True:
        ...
        while not found:
            ...
            while iter < 13 and not found:
                ...
                if geometry.Polygon(coordinates).contains(pnt) or last_round == True:
                    ...
                    #check whether enough space after x and y for patch_size
    ...
    def __getitem__(self, idx): #idx is filename
        ...
        (x,y), slide_filename = sample["coords"], sample["slide_filename"]
        slide_objects = self.slide_objects[slide_filename]
        slide, polygons = slide_objects["slide"], slide_objects["polygons"]
